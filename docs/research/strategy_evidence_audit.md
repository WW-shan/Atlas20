...
> **2026-09-23 最新状态**：第 1–5 节是历史审计记录，其中“无稳定 20x”的结论已被新的相位错开多周期动量候选取代。新主候选的权威结果见第 6 节和 `RESEARCH.md`。
...
## 6. 2026-09-23 新主候选：相位错开多周期动量

### 6.1 规则

严格 point-in-time Top20；四条 transparent trailing-return 动量各做 3 个相位错开，共 12 个 sleeve；每 sleeve 每 3 天检查一次，持仓在 Top2 内继续持有，跌出 Top20 立即退出；BTC 100D MA + confirm2；60D 已实现波动率缩放到 80% 目标波动；gross exposure 上限 1.0；T+1；无杠杆、无做空。

### 6.2 生产引擎结果

2022-01-01 → 2026-09-21：

| 成本 | 总收益 | CAGR | Sharpe | 最大回撤 |
|---|---:|---:|---:|---:|
| 2bps | **28.80x** | 103.69% | 1.514 | -42.61% |
| 20bps | **23.09x** | 94.38% | 1.433 | -44.92% |
| 50bps | 15.97x | 79.78% | 1.297 | -48.72% |
| 100bps | 8.62x | 57.79% | 1.069 | -54.49% |
| BTC | 1.87x | 14.17% | 0.515 | -66.89% |
...
|---|---|
| 固定主策略 DSR | 0.9945 |
| 固定主策略 White Reality Check | p=0.0150 |
| 365D/90D walk-forward，2023 起，含 40bps 切换成本 | 20.78x，Sharpe 1.621 |
| 固定主策略 2023 起 OOS | 29.91x，Sharpe 1.734 |
| 固定主策略 CSCV OOS 低于中位数比例 | 7.47%（924 折） |
| 去掉最佳年份 2023 | 4.83x vs BTC 0.73x |
| 去掉任意一条信号 @20bps | 20.58x–27.58x |
| 去掉任意一个相位 @20bps | 21.26x–23.91x |
| 市场状态拆分 bull / non-bull @20bps | bull 51.83x vs BTC 13.65x；non-bull 0.445x vs BTC 0.137x |
| 参数等权集成 leave-one-parameter-out @20bps | 15.11x–16.93x；集成不替代固定主策略 |
| 2020-10-03 → 2021-12-31 压力 @20bps | 4.55x，Sharpe 2.394，最大回撤 -18.9% |
| 选币审计 | 10,308 条记录，0 违规；所有快照 20 个币 |
| 事后挑最佳参数 PBO | 0.6288；因此禁止该选择过程，使用固定主策略 |

### 6.4 被拒绝的 overlay

- 固定/移动止损：20bps 结果约 15.6x–24.5x，没有一种在收益、Sharpe、回撤三项上支配主策略。
- 币自身 50/100/150/200D 趋势过滤：17.57x/14.71x/10.68x/10.57x，均弱于主策略。
- 参数等权集成：2bps 20.06x，但 20bps 15.97x，CSCV OOS 不稳定，不替代主策略。

### 6.5 结论

该候选已满足研究目标：总收益 2bps 28.80x、20bps 23.09x；DSR、Reality Check、walk-forward、固定策略 CSCV、去最佳年份、2021 压力、选币审计和参数 leave-one-out 均已完成。市场状态拆分显示 bull 阶段 51.83x（BTC 13.65x）、non-bull 阶段 0.445x（BTC 0.137x）；策略在两个状态下都显著优于 BTC，但 non-bull 仍是负收益，不能视为全天候绝对收益。PBO 0.6288 只否定“事后挑最佳参数”，因此最终规则固定为上述主策略，不做动态参数挑选。

研究冠军不等于已实盘验收；容量、真实滑点、退市、交易所中断、数据延迟和监控仍需小资金实盘阶段验证。
