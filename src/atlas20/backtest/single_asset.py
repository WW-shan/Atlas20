...
def simulate_single_asset_weight_targets(
    asset_returns: pd.DataFrame,
    target_assets: pd.Series,
    target_weights: pd.Series,
    *,
    ...
    """Simulate sparse fractional target events with drifting weights.

    Unlike :func:`simulate_single_asset_weights`, this function treats a
    non-null row in ``target_assets``/``target_weights`` as a rebalance event
    and does **not** forward-fill the target to every calendar day. Between
    ...
    Non-event days must have both fields null/NaN. This distinction is
    important: a strategy that only rebalances every 14 days must not be
    charged daily rebalancing costs as if it were rebalanced to a constant
    weight every day.
    """
    if total_cost_bps < 0:
        raise ValueError("total_cost_bps must be non-negative")
    ...
    returns = asset_returns.copy().sort_index()
    assets = target_assets.reindex(returns.index).astype("object")
    weights = pd.to_numeric(target_weights.reindex(returns.index), errors="coerce")
    asset_present = assets.notna()
    weight_present = weights.notna()
    if (asset_present != weight_present).any():
        raise ValueError("target_assets and target_weights must be present together")
    if weight_present.any() and ((weights[weight_present] < 0.0) | (weights[weight_present] > 1.0)).any():
        raise ValueError("target weights must be between 0 and 1")
    ...
    for row_position, date in enumerate(returns.index):
        if row_position > 0:
            raw_asset = asset_values[row_position - 1]
            raw_weight = weight_values[row_position - 1]
            if not pd.isna(raw_asset):
                pending_asset = None if str(raw_asset) in {"", "__cash__"} else str(raw_asset)
                pending_weight = float(raw_weight) if pending_asset is not None else 0.0
                pending_target_set = True
        ...
        # Match the production engine's post-return drift: a fractional
        # position is not silently reset to its target weight every day.
        gross_multiplier = 1.0 + gross_return
        if gross_multiplier > 0.0:
            current_weight = current_weight * (1.0 + asset_return) / gross_multiplier
        else:
            current_weight = 0.0
