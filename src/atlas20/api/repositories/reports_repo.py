...
class ReportsRepo:
    ...
    def list(self, *, sort: str = "recent") -> builtins.list[ReportFile]:
        stmt = select(ReportFile)
        if sort == "oldest":
            stmt = stmt.order_by(col(ReportFile.generated_at).asc(), col(ReportFile.id).asc())
        elif sort == "size":
            stmt = stmt.order_by(col(ReportFile.size_bytes).desc(), col(ReportFile.id).asc())
        elif sort == "type":
            stmt = stmt.order_by(col(ReportFile.kind).asc(), col(ReportFile.generated_at).desc())
        else:
            stmt = stmt.order_by(col(ReportFile.generated_at).desc(), col(ReportFile.id).asc())
    ...
    def upsert(self, report: ReportFile) -> ReportFile:
        ...
        if report.run_id is None:
            stmt = stmt.where(col(ReportFile.run_id).is_(None), ReportFile.path == report.path)
        else:
            stmt = stmt.where(ReportFile.run_id == report.run_id)
        existing = self._s.exec(stmt.order_by(col(ReportFile.generated_at).desc(), col(ReportFile.id).desc())).first()
        if existing is None:
    ...
    def by_run(self, run_id: str) -> builtins.list[ReportFile]:
        stmt = select(ReportFile).where(ReportFile.run_id == run_id).order_by(col(ReportFile.generated_at).desc())
        return builtins.list(self._s.exec(stmt).all())
    ...
    def by_run_kind(self, run_id: str | None, kind: str) -> ReportFile | None:
        ...
        stmt = (
            ...
            .order_by(col(ReportFile.generated_at).desc(), col(ReportFile.id).desc())
        )
