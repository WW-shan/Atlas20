...
def compute_leader_coin_scores(
    ...
    Note on the short-horizon term: subtracting a per-sector constant (such as
    the sector's own return) from every member leaves the *within-sector* rank
    untouched, so a "relative to sector" rank is mathematically identical to
    the raw momentum rank and merely double-counts it. A 7-day acceleration
    term is used instead: it is a genuinely different horizon, and it is what
    surfaces a coin at the moment leadership starts to rotate.
    """
    if not coin_ids:
    ...
    ret_7 = trailing_return(market.price, rebalance_date, 7).reindex(coin_ids)
    ...
    ranked['ret_7_rank'] = frame['ret_7'].rank(pct=True)
