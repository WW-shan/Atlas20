...
在 **真实 point-in-time Top20 + 无杠杆 + 现货** 约束下，当前最稳健的研究候选已经不是单个参数，而是 **7D/14D × 50%/60%/70%/80% 共 8 个波动率目标版本的固定等权集成**。每个版本仍使用 balanced top1、75D/3 自身止损和 BTC 100D MA 闸门。旧的固定起点高收益组合仍保留作上尾对照：
